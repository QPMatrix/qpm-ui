import { type QpRegistryItem } from "../schemas/registry-item.schema";

/** Horizontal or vertical rule built on Base UI's separator primitive. */
export const separator: QpRegistryItem = {
  name: "separator",
  type: "primitive",
  description: "Horizontal or vertical rule built on Base UI's separator primitive.",
  version: "0.1.0",
  files: [
    {
      path: "packages/ui/src/components/ui/separator.tsx",
    },
  ],
  dependencies: ["@base-ui/react"],
  registryDependencies: ["cn"],
  aliases: {
    ui: "@/components/ui",
    utils: "@/lib/utils",
  },
  tokenDependencies: ["border-default"],
  accessibility: {
    status: "partial",
    wcagLevel: "2.2-AA",
    interactive: false,
    keyboardTested: false,
    focusManaged: false,
    notes:
      "Installed unmodified from shadcn/ui on Base UI. Keyboard and ARIA behaviour is owned and tested upstream by Base UI; QPMatrix has not yet run its own audit against docs/standards/accessibility.md.",
  },
  supportedPlatforms: ["web"],
  tags: ["layout"],
};
