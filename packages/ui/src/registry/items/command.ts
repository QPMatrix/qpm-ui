import { type QpRegistryItem } from "../schemas/registry-item.schema";

/** Base UI command primitive, installed from shadcn/ui and bound to @qpmatrix/tokens. Exports 9 symbol(s). */
export const command: QpRegistryItem = {
  name: "command",
  type: "primitive",
  description:
    "Base UI command primitive, installed from shadcn/ui and bound to @qpmatrix/tokens. Exports 9 symbol(s).",
  version: "0.1.0",
  files: [{ path: "packages/ui/src/components/ui/command.tsx" }],
  dependencies: ["cmdk", "lucide-react"],
  registryDependencies: ["cn", "dialog", "input-group"],
  aliases: {
    ui: "@/components/ui",
    utils: "@/lib/utils",
  },
  tokenDependencies: [
    "border-default",
    "border-interactive",
    "fg-muted",
    "fg-primary",
    "radius-lg",
    "radius-sm",
    "radius-xl",
    "surface-secondary",
    "surface-tertiary",
  ],
  accessibility: {
    status: "partial",
    wcagLevel: "2.2-AA",
    interactive: true,
    keyboardTested: true,
    focusManaged: true,
    notes:
      'Installed unmodified from shadcn/ui on Base UI. Keyboard and ARIA behaviour is owned and tested upstream by Base UI; QPMatrix has not yet run its own audit against docs/standards/accessibility.md, so the status is "partial" rather than "audited". Its @qpmatrix/ui colours resolve through styles/qpmatrix.css, so the token layer is already covered by bun run a11y:contrast.',
  },
  supportedPlatforms: ["web"],
  tags: ["navigation", "search"],
};
