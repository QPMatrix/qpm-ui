 ...
 *
 * Writing a @qpmtx/ui component by hand means re-deriving the same eight
...
${primitiveDependency.join("\n")}
  ...
  // TODO(scaffold): @qpmtx/tokens custom-property names WITHOUT the leading
...
Next:
  ...
  2. bun run --filter @qpmtx/ui test          # the contract suite must pass
  3. bun run --filter @qpmtx/ui storybook     # preview it, check the a11y panel
  4. bun run --filter @qpmtx/ui registry:check ${kebab}
  5. bun run --filter @qpmtx/ui registry:build
`);
}
